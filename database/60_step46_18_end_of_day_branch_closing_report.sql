-- ============================================================================
-- Tiny POS - Step 46.18: Consolidated End of Day / Branch Closing Report
-- Run once after Step 46.17 / migration 59.
--
-- Extends the existing end-of-day RPC without changing operational records.
-- Adds receipt counts, separated cash/bank/card/credit totals, expenses,
-- gross profit, refund detail and expense detail for one-file Print/Export.
-- ============================================================================
  ...
  return (
    ...
    summary_rows as (
      select
        ...
        (select count(*) from scoped_returns r where r.currency = c.currency) as refund_count,
        ...
        coalesce((select sum(e.amount) from scoped_cash e where e.currency = c.currency and e.direction = 'expense' and e.affects_profit), 0) as expenses,
        coalesce((select sum(p.amount) from scoped_payments p where p.currency = c.currency and lower(p.method::text) = 'cash'), 0) as cash_received,
        coalesce((select sum(p.amount) from scoped_payments p where p.currency = c.currency and lower(p.method::text) in ('bank', 'khqr')), 0) as bank_received,
        coalesce((select sum(p.amount) from scoped_payments p where p.currency = c.currency and lower(p.method::text) = 'card'), 0) as card_received,
        coalesce((select sum(p.amount) from scoped_payments p where p.currency = c.currency and lower(p.method::text) = 'other'), 0) as other_received,
        coalesce((select sum(s.credit_amount) from scoped_sales s where s.currency = c.currency), 0) as credit_sales,
        coalesce((select sum(s.gross_profit) from scoped_sales s where s.currency = c.currency), 0)
          - coalesce((select sum(r.profit_reversal) from scoped_returns r where r.currency = c.currency), 0) as gross_profit,
        -- Keep the previous fields for existing UI/backward compatibility.
    ...
    refund_detail as (
      select
        ...
        coalesce(cu.name, 'Walk-in') as customer_name,
        coalesce(pr.full_name, pr.email, 'POS Staff') as processed_by_name,
      ...
      left join public.profiles pr on pr.id = r.processed_by
    ...
    expense_detail as (
      select
        ...
        e.remark,
        e.affects_profit,
        coalesce(pr.full_name, pr.email, 'POS Staff') as created_by_name
      from scoped_cash e
    ...
    select jsonb_build_object(
      ...
      'receipt_counts', jsonb_build_object(
        'sales', (select count(*) from scoped_sales),
        'refunds', (select count(*) from scoped_returns)
      ),
      ...
      'refunds_detail', coalesce((select jsonb_agg(to_jsonb(d) order by d.processed_at desc) from refund_detail d), '[]'::jsonb),
      'expenses_detail', coalesce((select jsonb_agg(to_jsonb(d) order by d.entry_at desc) from expense_detail d), '[]'::jsonb),
